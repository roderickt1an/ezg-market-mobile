    ...
    <van-row style="overflow-x: hidden">
      ...
      />
        <van-row>
            ...
            />
            <van-cell-group style="width:80%;margin:auto;margin-top:30px">
              <van-cell title="公司名称" v-model="companyName" @click="openCompanySearch"/>
              <van-cell title="归属客户" v-model="belongClient"/>
              <van-cell title="跟进类型" v-model="followTypeName" placeholder="请选择" @click="openType" v-if="isShow"/>
              <van-field
                v-model="followMsg"
                label="跟进内容"
                type="textarea"
                placeholder="请输入跟进内容！(必填项)"
                autosize
              />
            </van-cell-group>
